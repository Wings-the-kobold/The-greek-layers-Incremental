    ...
    tabFormat: {
      ...
      "Upgrade Tree": {
        content: [
            ...
            Row 4 Upgrades unlock aat 10 total Repression completions
            `}
        ],
        "blank",
        "blank",
...
//The Negative effects of The Blue path is 20% weaker, but C3's effect is divided by 3.3  
   ...
   canAfford() {
    let pathUnlock = false
    let otherPath = false
    if (hasUpgrade("R",33) && hasUpgrade("R",34)) pathUnlock = true
    if (hasUpgrade("R",43) || hasUpgrade("R",45)) otherPath = true
    if (pathUnlock == true && otherPath == false) return true
    },
  },
  ...
  46: {
    ...
    canAfford() {
      if (hasUpgrade("R",36) && hasUpgrade("R",37) ) return true
    },
